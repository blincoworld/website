...
const submit=process.argv.includes('--submit');if(submit)assert.equal(base,'https://piersblinco.com');
const admin='https://business-os.pbwebonlinesales.workers.dev';
...
try{
 ...
 for(const width of [1440,375,412,768]){
  ...
  console.log('PASS '+width+'px: supplied film playback, seek/end, native controls, layout and touch targets');
 }
 ...
 if(submit){
  ...
  const raw=(await fs.readFile('/home/piers-blinco/business-os/.dev.vars.messenger-production-token','utf8')).trim();const token=(raw.includes('=')?raw.slice(raw.indexOf('=')+1):raw).trim().replace(/^['"]|['"]$/g,'');
  ...
  await fs.writeFile('/tmp/property-release-private/smoke-lead.json',JSON.stringify({id:lead.id,business_name:business,notification_status:saved.lead.notification_status}),{mode:0o600});
  console.log('PASS production form success and real Property Business lead:',lead.id,'notification:',saved.lead.notification_status);
  ...
  for(const module of ['dashboard','sales','prospecting','content']){await click(`[data-module="${module}"]`);await wait("!document.querySelector('.module-loading') && !document.querySelector('.module-error')");await sleep(700);assert(await js("document.querySelector('.workspace').textContent.trim().length>50"));}
  console.log('PASS production authentication, four Property Business areas, full confirmation cancelled, existing dashboard/Sales/Prospecting/Content');
 }
